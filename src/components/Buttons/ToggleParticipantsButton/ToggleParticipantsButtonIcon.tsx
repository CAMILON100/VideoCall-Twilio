...
export default function ToggleParticipantsButtonIcon(props: { className?: string }) {
  const classes = useStyles();
  const [shouldAnimate, setShouldAnimate] = useState(false);
  const { isChatWindowOpen, setIsChatWindowOpen, conversation, hasUnreadMessages } = useChatContext();
  const { setIsBackgroundSelectionOpen } = useVideoContext();

  const toggleParticipantsWindow = () => {
    try {
      let particpantList = document.getElementById('participant-list');
      console.log(particpantList);
      if (window.getComputedStyle(particpantList!).display === 'none') {
        particpantList!.style.display = '';
      } else {
        particpantList!.style.display = 'none';
      }
    } catch {
      console.error('Participant list not exists');
    }
  };
  ...
  return (
    <Button
      onClick={toggleParticipantsWindow}
